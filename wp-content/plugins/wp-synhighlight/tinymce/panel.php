...
<?php if (! get_option('wp_synhighlight_disable_editarea')) :	?>
...
function displayCodeTab() {
	...
	editAreaLoader.init({
		...
		,language: "<?php $lang = substr(WPLANG, 0, 2); 
		if (in_array($lang, array('bg', 'cs', 'de', 'dk', 'en', 'eo', 'es', 'fi', 'fr', 'hr', 'it', 'ja', 'mk', 'nl', 'pl', 'pt', 'ru', 'sk', 'zh'))) {
			echo $lang;
		} else {
			echo "en";
		} ?>"
...
function resizeTextArea() {
	...
	<?php if (! get_option('wp_synhighlight_disable_editarea')) : ?>
			editAreaLoader.init({
				...
				,allow_toggle: true
				,word_wrap: true
				,language: "<?php echo substr('en', 0, 2); ?>"
				,syntax: getSelectValue('lang')
			});			
  ...
  <div class="panel_wrapper">
    <div id="options_panel" class="panel current" style="height: auto">
      <table border="0">
        ...
        <tr>
          <th align="right" scope="row"> <p>{#WPSynHighlight_dlg.Container}:</p>
          </th>
          <td width="3">&nbsp;</td>
	...
	id="sourcecode"> Enter code here</textarea>
    </div>
  </div>
  <p>
